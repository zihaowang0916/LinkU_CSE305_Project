...
<div class="tab-pane fade in active" id="aParCus">
    ...
    else
    {
    	%>
    		<h1>information</h1>
                	...
                	query = "select * from ( SELECT C.UserId as UserID, Sum(BookingFee) as bookingfee from CustomerData C, ProfileData P, DateData D WHERE C.UserId='" + userid +"' and C.UserId = P.UserId and (P.ProfileID = D.Profile1 or P.ProfileID = D.Profile2) GROUP BY C.UserId ) temp inner join CustomerData C1 on c1.UserID = temp.UserID";
                	
                	//"select userID, LastName, FirstName, sum(BookingFee) as bookingfee from ((select temp.userid as userid, profiledata.ProfileID as profileid , lastname, firstname   from (select * from customerdata) temp INNER JOIN profiledata ON profiledata.userID = temp.userID ) temp2 INNER JOIN datedata ON (datedata.profile1 = temp2.profileID or datedata.profile2 = temp2.profileID )) where userid = '"+ userid +"'";
                    java.sql.Statement stmt1=conn.createStatement();
                	java.sql.ResultSet rs1 = stmt1.executeQuery(query);
...
<div class= "tab-pane fade" id="datingRec">
				<form method="post"> <!--action="http://localhost:8080/305/save_edit.jsp">  -->
				<table>
				<tr><td>User ID:</td><td>
					<select name="search_user" id = "search_user">
